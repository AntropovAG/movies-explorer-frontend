...
function Register({ onRegister, message, onMessageReset }) {
  ...
  return (
    <form className="register-form" name="register" onSubmit={handleSubmit} noValidate>
      ...
      <input className="register-form__input" id="name-input" type="name" name="name" placeholder='' minLength='2' maxLength='30' onChange={handleNameChange} required/>
      <span className="register-form__input-error name-input-error">{nameError}</span>
      ...
      <span className="register-form__input-error email-input-error">{emailError}</span>
      ...
      <button className="register-form__authorize-button" type="submit" disabled={( isValidName===false || isValidEmail===false  || isValidPassword===false  )}>Зарегистрироваться</button>
